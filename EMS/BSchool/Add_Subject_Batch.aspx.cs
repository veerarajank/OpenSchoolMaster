...
public partial class Add_Subject_Batch : System.Web.UI.Page
{
   protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string course_id = Convert.ToString(drp_course.SelectedValue);;
    ...
    protected void BindListView(string academic_id, string course_id)
    {
        ...
        if (dt_data != null)
        {
            if (dt_data.Rows.Count > 0)
            {
                ...
                drp_course.SelectedValue = Convert.ToString(course_id);
            }
            else
            {
                ies = new IES_Generic_Function();
