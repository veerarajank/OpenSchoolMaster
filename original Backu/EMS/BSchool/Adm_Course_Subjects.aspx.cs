...
public partial class Adm_Course_Subjects : System.Web.UI.Page
{
    ...
    protected void lnk_edit_Command(object sender, CommandEventArgs e)
    {
        ...
        if (e.CommandName == "edit")
        {
            if (Convert.ToBoolean(Session["Access_Edit"]) == true)
            {
                ...
                Response.Redirect("Adm_Course_Subjects.aspx?mode=edit&id=" + id);
            }
        }
    }
    protected void lnk_search_Command(object sender, CommandEventArgs e)
    {
        ...
        if (e.CommandName == "view")
        {
            if (Convert.ToBoolean(Session["Access_View"]) == true)
            {
                ...
                Response.Redirect("Adm_Course_Subjects.aspx?mode=view&id=" + id);
            }
        }
    }
    ...
    {
        Response.Redirect("Adm_Course_Subjects.aspx?mode=search");
    }
    protected void btn_update_ServerClick(object sender, EventArgs e)
    {
        ...
        DataTable dt_result = ies.Fn_ExecutiveSql_Datatable("UDP_Subjects", 1, 1);
        if (dt_result != null && dt_result.Rows.Count > 0)
        {
            if (Convert.ToString(dt_result.Rows[0]["Error"]) == "0")
            {
                Response.Redirect("Adm_Course_Subjects.aspx");
            }
        }
    }
    protected void btn_del_ServerClick(object sender, EventArgs e)
    {
        ...
        if (Convert.ToBoolean(Session["Access_Trash"]) == true)
        {
            ...
            ies.Fn_ExecutiveSql("UDP_Subjects", 1, 1);
            Response.Redirect("Adm_Course_Subjects.aspx");
        }
    }
    ...
    {
        Response.Redirect("Adm_Course_Subjects.aspx?mode=add");
    }
}