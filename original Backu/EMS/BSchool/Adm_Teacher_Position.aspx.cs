...
public partial class Adm_Teacher_Position : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ies.Fn_Executive_ListWithSelect_WithTitle(drp_category,"--Select Category--", "select id,Name from tbl_teacher_category where status=1 order by Name", "id", "Name");
            ies = new IES_Generic_Function();
            ies.Fn_Executive_ListWithSelect_WithTitle(drp_edit_category, "--Select Category--", "select id,Name from tbl_teacher_category where status=1 order by Name", "id", "Name");
    ...
    protected void lnk_edit_Command(object sender, CommandEventArgs e)
    {
        ...
        if (e.CommandName == "edit")
        {
            if (Convert.ToBoolean(Session["Access_Edit"]) == true)
            {
                ...
                Response.Redirect("Adm_Teacher_Position.aspx?mode=edit&id=" + id);
            }
        }
    }
    protected void lnk_search_Command(object sender, CommandEventArgs e)
    {
        ...
        if (e.CommandName == "view")
        {
            if (Convert.ToBoolean(Session["Access_View"]) == true)
            {
                ...
                Response.Redirect("Adm_Teacher_Position.aspx?mode=view&id=" + id);
            }
        }
    }
    protected void btn_new_ServerClick(object sender, EventArgs e)
    {
        Response.Redirect("Adm_Teacher_Position.aspx?mode=add");
    }
    protected void BindListView()
    {
        ...
        DataTable dt_data = ies.Fn_ExecutiveSql_Datatable("UDP_Teacher_Position", 1, 1);
        if (dt_data != null)
        {
    ...
    protected void BindDisplay()
    {
        ...
        if (dt_data != null && dt_data.Rows.Count > 0)
        {
            ...
            if (drp_edit_category.Items.FindByValue(Convert.ToString(dt_data.Rows[0]["category_id"])) != null)
            {
                drp_edit_category.SelectedValue = Convert.ToString(dt_data.Rows[0]["category_id"]);
            }
            ...
            lbl_category.InnerText = Convert.ToString(dt_data.Rows[0]["category_name"]);
    ...
    {
        Response.Redirect("Adm_Teacher_Position.aspx?mode=search");
    }
    ...
    protected void btn_del_ServerClick(object sender, EventArgs e)
    {
        ...
        if (Convert.ToBoolean(Session["Access_Trash"]) == true)
        {
            ...
            ies.Fn_ExecutiveSql("UDP_Teacher_Position", 1, 1);
            Response.Redirect("Adm_Teacher_Position.aspx");
        }
    }
    protected void btn_createnew_ServerClick(object sender, EventArgs e)
    {
        ...
        string category_id = Convert.ToString(drp_category.SelectedItem.Value);
